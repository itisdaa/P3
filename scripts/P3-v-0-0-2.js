...
.then((response) => {
   ...
   document.querySelector('.prefered-rate-input input').min=Math.min(...availableRates);
   document.querySelector('.prefered-rate-input input').max=Math.max(...availableRates);
});
...
let themeDB = new Object();
fetch("https://jarfa.000webhostapp.com/Theme-Database.json")
.then((response) => {
   return response.json();
}).then((jsondata) => {
   themeDB = jsondata;
...
document.querySelector('.theme-change').onclick = () => {
  indexThemeDB++;
  indexThemeDB=arrayIndexLoop(indexThemeDB,themeDB);
...
form.oninput = () => {
  ...
  makerInput = {
    ...
    location:pos,
    rate: document.querySelector('.prefered-rate-input input').value
  };
...
function updateSuggestion() {
  if (-1 < z && z < suggestedIndexDB.length) {
    ...
    document.querySelector('legend#prefered-rate-legend span').textContent='Rs '+makerInput.rate+'/g';
